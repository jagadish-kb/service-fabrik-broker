...
const cloudProvider = require('../iaas').cloudProvider;
...
class DirectorManager extends BaseManager {
  ...
  ensureSecurityGroupExists(spaceGuid) {
    return cloudProvider
      .getSecurityGroup(`${CONST.SERVICES_SECURITY_GROUP.PREFIX}_${spaceGuid}`)
      .then(securityGroup => {
        if (securityGroup !== undefined) {
          logger.info(`Found Security group for space: ${spaceGuid}`);
          return;
        }
        logger.info(`Security group for space: ${spaceGuid} not found. Creating it.`);
        return cloudProvider
          .createSecurityGroup(`${CONST.SERVICES_SECURITY_GROUP.PREFIX}_${spaceGuid}`, `Service Fabrik Bosh Services Network rules for Space ${spaceGuid}`)
          .then(securityGroup => Promise.all([
            this.createSecurityRules(spaceGuid, securityGroup),
            this.createCloudConfigForSpace(spaceGuid, securityGroup)
          ]));
      });
  }

  createSecurityRules(spaceGuid, securityGroup) {
    return Promise.try(() => {
      const securityRules = _.cloneDeep(BoshDirectorClient.getInfrastructure().bosh_services_security_rules);
      _.each(securityRules, rule => {
      ...
      _.each([
        ...
        const selfRule = {
          ...
          ethertype: 'IPv4',
          protocol: 'tcp',
          portRangeMin: ports[0],
          portRangeMax: ports[1],
          remoteGroupId: securityGroup.id,
          name: `${CONST.SERVICES_SECURITY_GROUP.PREFIX}_${spaceGuid}`
        };
        securityRules.push(selfRule);
      });
      return cloudProvider
  ...
  createCloudConfigForSpace(spaceGuid) {
    const directorName = this.director.constructor.getActivePrimary()[0].name;
    return cloudConfigManager
      .getCloudConfig(directorName, CONST.CLOUD_CONFIG_DEFAULT_NAME)
      .then(cloudConfig => {
        const createCloudConfig = () => {
          const serviceBoshNetworkDefinition = _.head(_.filter(cloudConfig.networks, (network) => network.name === this.networkName));
          logger.info('Service bosh network -- ', serviceBoshNetworkDefinition);
          const cloudConfigSg = _.cloneDeep(serviceBoshNetworkDefinition);
          cloudConfigSg.name = `${this.networkName}_${spaceGuid}`;
          _.each(cloudConfigSg.subnets, (subnet) => _.set(subnet, 'cloud_properties.security_groups', [`${CONST.SERVICES_SECURITY_GROUP.PREFIX}_${spaceGuid}`]));
          const networkDefinition = {
            networks: [cloudConfigSg]
          };
          return networkDefinition;
        };
        return cloudConfigManager
          .fetchCloudConfigAndUpdate(directorName, `${this.networkName}_${spaceGuid}`, createCloudConfig)
          .tap(() => logger.info('Successfully updated cloud config....'));
      });
  }
  ...
  generateManifest(deploymentName, opts) {
    ...
    const boshServicesNetwork = _.each(networks[this.networkName], (net => net.name = `${net.name}_${opts.space_guid}`));
    const context = new EvaluationContext({
      index: index,
      header: header,
      cpi: this.director.cpi,
      networks: boshServicesNetwork,
